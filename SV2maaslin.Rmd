...
cat("physeqSV2 - Samples:", nsamples(physeqDC), 
    "OTUs:", ntaxa(physeqDC), "\n")
...
cat("physeqSV2 with metadata - Samples:", nsamples(physeqDC), 
    "OTUs:", ntaxa(physeqDC), "\n")
...
fit_data <- Maaslin2(
  ...
  output         = "SV2_runs/outMaaslin2_LOSvsControl",
...
sig_res <- read_tsv("SV2_runs/outMaaslin2_LOSvsControl/significant_results.tsv")
...
write.csv(final_res, "SV2_runs/Top_1.features_with_taxonomy.csv", row.names = FALSE)
...
write.csv(sig_taxa, "SV2_runs/1.Significant_features_taxonomy_ranked.csv", row.names = FALSE)
...
sig_taxa <- read.csv("SV2_runs/1.Significant_features_taxonomy_ranked.csv")
...
fit_sc <- Maaslin2(
  ...
  output         = "SV2_runs/out4Maaslin2_Saureus_vs_Control",
...
fit_gc <- Maaslin2(
  ...
  output         = "SV2_runs/out2Maaslin2_GutLOS_vs_Control",
...
fit_sg <- Maaslin2(
  ...
  output         = "SV2_runs/out3Maaslin2_Saureus_vs_GutLOS",
  fixed_effects  = c("DiseaseCausative", "Timeperiods"),
  random_effects = c("Patient.ID"),
...
out_dir <- "SV2_runs/out5Maaslin2_EcoliControl"
