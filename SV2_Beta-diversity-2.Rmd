...
stats_disease  <- dplyr::bind_rows(lapply(results, `[[`, "stats"))

wrap_plots(plots, nrow = 1)
print(stats_disease )
write_xlsx(stats_disease , "figs/SV2-PCoA_time_window_stats.xlsx")
...
ggsave(
  filename = "figs/SV2_PCoA_time_windows.png",
  plot = combined,
  width = 14,
  height = 4,
  dpi = 300
...
## added
stats_subgroup <- dplyr::bind_rows(lapply(results_subgroup, `[[`, "stats"))
...
## ---------------------------
ggsave(
  "figs/SV2-PCoA_subgroup_time_windows.png",
  final_subgroup,
  width = 14,
  height = 4,
  dpi = 300
...
## ---------------------------
## global pathogen levels
## ---------------------------
pathogen_levels <- levels(sample_data(phy.norm)$Bloodculture_isolated_pathogen)

## create named palette
pathogen_palette <- setNames(
  scales::hue_pal(l = 60, c = 150)(length(pathogen_levels)),
  pathogen_levels
)

## shared color scale (locked)
pathogen_colors <- scale_color_manual(
  values = pathogen_palette,
  drop = FALSE
)
...
plots <- lapply(results, `[[`, "plot")
stats_pathogen  <- dplyr::bind_rows(lapply(results, `[[`, "stats"))

## ---------------------------
## extract ONE legend
## ---------------------------
## create legend from FULL dataset (not subsets)
legend_plot <- plot_ordination(
  phy.norm,
  ordinate(phy.norm, "PCoA", "bray"),
  color = "Bloodculture_isolated_pathogen"
) +
  geom_point() +
  pathogen_colors +
...
## ---------------------------
ggsave(
  "figs/SV2_beta_Pathogen_PCoA_time_windows.png",
  final_plot,
  width = 14,
  height = 4,
  dpi = 300
...
## new set of metrics for the new PCoA plots 
```{r}
library(writexl)
write_xlsx(
  list(
    DiseaseStatus = stats_disease,
    Subgroups = stats_subgroup,
    Pathogens = stats_pathogen
  ),
  "figs/SV2_PERMANOVA_all_results.xlsx"
)
...
stats_disease  <- round_stats(stats_disease)
stats_subgroup <- round_stats(stats_subgroup)
stats_pathogen <- round_stats(stats_pathogen)
