...
cat("physeqDC - Samples:", nsamples(physeqDC), 
    "OTUs:", ntaxa(physeqDC), "\n")
...
# Recode DiseaseStatus properly
meta_maaslin$DiseaseStatus <- as.character(meta_maaslin$DiseaseStatus)
meta_maaslin$DiseaseStatus[meta_maaslin$DiseaseStatus == "1"] <- "LOS"
meta_maaslin$DiseaseStatus[meta_maaslin$DiseaseStatus == "0"] <- "Control"

# Then make it a factor with correct order
meta_maaslin$DiseaseStatus <- factor(meta_maaslin$DiseaseStatus,
                                     levels = c("Control", "LOS"))

# Check
table(meta_maaslin$DiseaseStatus)
levels(meta_maaslin$DiseaseStatus)
...
fit_data <- Maaslin2(
  ...
  output         = "SV1_runs/outMaaslin2_LOSvsControl",
...
sig_res <- read_tsv("SV1_runs/outMaaslin2_LOSvsControl/significant_results.tsv")
...
write.csv(final_res, "SV1_runs/Top_1.features_with_taxonomy.csv", row.names = FALSE)
...
write.csv(sig_taxa, "SV1_runs/1.Significant_features_taxonomy_ranked.csv", row.names = FALSE)
...
sig_taxa <- read.csv("SV1_runs/1.Significant_features_taxonomy_ranked.csv")
...
ref_time <- levels(df_meta_sc$Timeperiods)[1]
fit_sc <- Maaslin2(
  input_data     = t(otu_table(phy.sc)),
  input_metadata = df_meta_sc,
  output         = "SV1_runs/out4Maaslin2_Saureus_vs_Control_refOK",
  fixed_effects  = c("DiseaseCausative", "Timeperiods"),
  random_effects = c("Patient.ID"),
  reference      = c(paste0("Timeperiods,", ref_time), "DiseaseCausative,Control"),
  normalization  = "NONE",
  transform      = "NONE",
  analysis_method = "LM"
)
...
fit_gc <- Maaslin2(
  ...
  output         = "SV1_runs/out2Maaslin2_GutLOS_vs_Control",
...
fit_sg <- Maaslin2(
  ...
  output         = "SV1_runs/out3Maaslin2_Saureus_vs_GutLOS",
  fixed_effects  = c("DiseaseCausative", "Timeperiods"),
  random_effects = c("Patient.ID"),
...
out_dir <- "SV1_runs/out5Maaslin2_EcoliControl"
