...
ggsave("figs/alpha_LOS_vs_Control.png", height = 5, width = 12)
...
write.csv(pvals, "alpha_diversity_pvalues.csv", row.names = FALSE)
...
```
##Alpha Diversity for S.Aureus vs gram neg vs healthy:
...
# Save results to CSV
write.csv(results, "xls_out/21_alpha_diversity_pvalues.csv", row.names = FALSE)
...
# Save to CSV
write.csv(pval_df, "xls_out/2b_pvalue_results_metrics1.csv", row.names = FALSE)
...
# Save to CSV
write.csv(pval_df, "xls_out/3a_Ecoli_vs_Control_pvalues.csv", row.names = FALSE)
...
write.csv(pvalues_ecoli, "xls_out/3b_ecoli_pvalues.csv", row.names = FALSE)
...
#Compare each group separately over time:
#Richness:
```{r}
#gram neg only:
gn_alpha = subp1$data[subp1$data$DiseaseCausative == "gram.neg",]
#s.aureus only:
sa_alpha = subp1$data[subp1$data$DiseaseCausative == "s.aureus",]
#control only:
con_alpha = subp1$data[subp1$data$DiseaseCausative == "None",]
```

```{r}
#Richness-compare across time for gram neg only:
kruskal.test(data=gn_alpha, value ~ Timeperiods)
```

```{r}
#Richness-compare across time for s.aureus only:
kruskal.test(data=sa_alpha, value ~ Timeperiods)
```

```{r}
#Richness-compare across time for control only:
kruskal.test(data=con_alpha, value ~ Timeperiods)
```
...
#g.neg compare:
#kruskal pairwise comparison:
kruskalmc(gn_alpha$value, gn_alpha$Timeperiods)
krus<-with(gn_alpha,kruskal(value,Timeperiods,group=FALSE,p.adj="BH"))
krus$comparison

#wilcox pairwise comparison:
pairwise_result <- pairwise.wilcox.test(gn_alpha$value, gn_alpha$Timeperiods, p.adjust.method = "BH")
print(pairwise_result)
```
```{r}
#s.aureus compare:
#kruskal pairwise comparison:
kruskalmc(sa_alpha$value, sa_alpha$Timeperiods)
krus<-with(sa_alpha,kruskal(value,Timeperiods,group=FALSE,p.adj="BH"))
krus$comparison

#wilcox pairwise comparison:
pairwise_result <- pairwise.wilcox.test(sa_alpha$value, sa_alpha$Timeperiods, p.adjust.method = "BH")
print(pairwise_result)
```
```{r}
#control compare:
#kruskal pairwise comparison:
kruskalmc(con_alpha$value, con_alpha$Timeperiods)
krus<-with(con_alpha,kruskal(value,Timeperiods,group=FALSE,p.adj="BH"))
krus$comparison

#wilcox pairwise comparison:
pairwise_result <- pairwise.wilcox.test(con_alpha$value, con_alpha$Timeperiods, p.adjust.method = "BH")
print(pairwise_result)
```
...
```{r}
#gram neg only:
gn_alpha = subp2$data[subp2$data$DiseaseCausative == "gram.neg",]
#s.aureus only:
sa_alpha = subp2$data[subp2$data$DiseaseCausative == "s.aureus",]
#control only:
con_alpha = subp2$data[subp2$data$DiseaseCausative == "None",]
```
...
```{r}
#gram neg only:
gn_alpha = subp3$data[subp3$data$DiseaseCausative == "gram.neg",]
#s.aureus only:
sa_alpha = subp3$data[subp3$data$DiseaseCausative == "s.aureus",]
#control only:
con_alpha = subp3$data[subp3$data$DiseaseCausative == "None",]
```
...
print(pairwise_result)
```
From this we find no time points are significantly different for each LOS type group when separately compared. S.aureus is the closest. 

#Alpha Diversity for different causative pathogen groups:
Alter the labels:
```{r}
p$data$Causative_pathogen[p$data$Causative_pathogen == -99] <- "None"
```

```{r}
#plot observed metric across causative pathogen:
subp1 = ggplot(data=p$data[p$data$variable=="Observed",], aes(x=Causative_pathogen, y=value, fill= Causative_pathogen))+
geom_boxplot(alpha=0.3)+
geom_point(size=2) + theme_bw()+ 
  labs(x='Causative Pathogen', y= "Richness")+
scale_fill_brewer(palette="Dark2")+ 
  theme(legend.position="none")+
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1),
        axis.title.x = element_text(margin = margin(t = 15)),
...
#plot shannon index across causative pathogen:
subp2 = ggplot(data=p$data[p$data$variable=="Shannon",], aes(x=Causative_pathogen, y=value, fill=Causative_pathogen))+
geom_boxplot(alpha=0.3)+
geom_point(size=2) + theme_bw()+ 
  labs(x='Causative Pathogen', y= "Shannon index")+
scale_fill_brewer(palette="Dark2")+ 
  theme(legend.position="none")+
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1),
        axis.title.x = element_text(margin = margin(t = 15)),
...
#plot chao index across causative pathogen:
subp3 = ggplot(data=p$data[p$data$variable=="Chao1",], aes(x=Causative_pathogen, y=value, fill= Causative_pathogen))+
geom_boxplot(alpha=0.3)+
geom_point(size=2) + theme_bw()+ 
  labs(x='Causative Pathogen', y= "Chao1")+
scale_fill_brewer(palette="Dark2")+ 
  theme(legend.position="none")+
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1),
        axis.title.x = element_text(margin = margin(t = 15)),
...
#combine all into one plot:
plot_grid(subp1,subp2,subp3,ncol=3)
ggsave(path = "figs", filename = "alpha_causative.png", height = 7, width = 12)
```

Significance tests:
```{r}
#Richness-compare across causative pathogen:
kruskal.test(data=subp1$data, value ~ Causative_pathogen)
```
Pairwise:
```{r}
#kruskal pairwise:
kruskalmc(subp1$data$value, subp1$data$Causative_pathogen)
krus<-with(subp1$data,kruskal(value,Causative_pathogen,group=FALSE,p.adj="BH"))
krus$comparison

#wilcox pairwise:
pairwise_result <- pairwise.wilcox.test(subp1$data$value, subp1$data$Causative_pathogen, p.adjust.method = "BH")
print(pairwise_result)
```


```{r}
#Shannon-compare across causative pathogen:
kruskal.test(data=subp2$data, value ~ Causative_pathogen)
```
Pairwise:
```{r}
#kruskal pairwise:
kruskalmc(subp2$data$value, subp2$data$Causative_pathogen)
krus<-with(subp2$data,kruskal(value,Causative_pathogen,group=FALSE,p.adj="BH"))
krus$comparison

#wilcox pairwise:
pairwise_result <- pairwise.wilcox.test(subp2$data$value, subp2$data$Causative_pathogen, p.adjust.method = "BH")
print(pairwise_result)
```


```{r}
#Chao1-compare across causative pathogen:
kruskal.test(data=subp3$data, value ~ Causative_pathogen)
```
Pairwise:
```{r}
#kruskal pairwise:
kruskalmc(subp3$data$value, subp3$data$Causative_pathogen)
krus<-with(subp3$data,kruskal(value,Causative_pathogen,group=FALSE,p.adj="BH"))
krus$comparison

#wilcox pairwise:
pairwise_result <- pairwise.wilcox.test(subp3$data$value, subp3$data$Causative_pathogen, p.adjust.method = "BH")
print(pairwise_result)
```
...
# new 5a done
#  need to run 1269 for phy.red <- readRDS("phyloseq_disc_red.rds")
...
out_dir <- "Maaslin2_causative_pathogen_EcolivsControl"
...
## test run > 1556 run thn

```{r}
#D) Sensitivity: drop Timeperiods to check robustness

fit_clr_noTP <- Maaslin2(
  input_data    = t(otu_table(ps_genus)),
  input_metadata= df_meta_EC_N,
  output        = "M2_genus_CLR_noTP",
  fixed_effects = c("Bloodculture_isolated_pathogen"),
  random_effects= c("Patient.ID"),
  normalization = "CLR",
  transform     = "NONE",
  min_prevalence= 0.10,
...
```{r}
# extract counts from the phyloseq object
asv_counts_mat <- as(otu_table(phy.red_EC_N), "matrix")

# make sure samples are rows
if (taxa_are_rows(phy.red_EC_N)) {
  asv_counts_mat <- t(asv_counts_mat)
}
...
```{r}
# Use the ASV count table (integers), not RA
fit_zinb <- Maaslin2(
  input_data    = asv_counts_mat,
  input_metadata= df_meta_EC_N,
  output        = "M2_ASV_ZINB_prev10",
  ...
  min_prevalence= 0.10,
  min_abundance = 1,
  analysis_method = "ZINB"   # explicitly specify ZINB
)
...
```{r}
# B) RA input with arcsin-sqrt + prevalence filtering
fit_arc <- Maaslin2(
  input_data    = t(otu_table(ps_genus)),
  input_metadata= df_meta_EC_N,
  output        = "M2_genus_ARCSIN_prev20",
  fixed_effects = c("Bloodculture_isolated_pathogen","Timeperiods"),
  random_effects= c("Patient.ID"),
  normalization = "TSS",
  transform     = "AST",
...
```{r}
# A) RA input with CLR + prevalence filtering
fit_clr <- Maaslin2(
  input_data    = t(otu_table(ps_genus)),   # or ASV table if you want ASV-level
  input_metadata= df_meta_EC_N,
  output        = "M2_genus_CLR_prev10",
  fixed_effects = c("Bloodculture_isolated_pathogen","Timeperiods"),
  random_effects= c("Patient.ID"),
  normalization = "CLR",          # compositional
  transform     = "NONE",         # CLR already handles scale
  min_prevalence= 0.10,           # 10% of samples
  min_abundance = 1e-3,           # for RA inputs
...
library(phyloseq)
ps_genus <- tax_glom(phy.red_EC_N, taxrank="Genus")
Maaslin2(
  input_data = t(otu_table(ps_genus)),
  input_metadata = df_meta_EC_N,
  output = "Maaslin2_GenusLevel_new",
  fixed_effects = c("Bloodculture_isolated_pathogen","Timeperiods"),
  random_effects = c("Patient.ID")
)
...
##By disease type: editing prev code
```{r}
# Set output directory
outdir <- "new_Maaslin2_outputs"
if (!dir.exists(outdir)) dir.create(outdir)
df_input_metadata=data.frame(sample_data(phy.red))
#reformat the inputs as strings:
df_input_metadata$DiseaseCausative[df_input_metadata$DiseaseCausative == 1 ] <- "s.aureus"
df_input_metadata$DiseaseCausative[df_input_metadata$DiseaseCausative == 2 ] <- "gram.neg"
df_input_metadata$DiseaseCausative[df_input_metadata$DiseaseCausative == 0 ] <- "Control"

df_input_metadata$DiseaseCausative = factor(df_input_metadata$DiseaseCausative,
                                   levels = c("s.aureus", "gram.neg", "Control"))

#subset by pairs of LOS types:
phy.red_SG= subset_samples(phy.red, DiseaseCausative!= 0)
phy.red_SN= subset_samples(phy.red, DiseaseCausative!= 2)
phy.red_NG= subset_samples(phy.red, DiseaseCausative!= 1)

#create a list of the subsets:
phy_list_2=list(phy.red_SG, phy.red_SN, phy.red_NG)
names(phy_list_2)=c("SG","SN","NG")

#With time period added:
#compare pairwise across LOS type and time period:

for (i in 1:length(phy_list_2)) {
  out_dir <- paste0("Maaslin2_diseasetype_output2_", names(phy_list_2)[i])
  print(out_dir)
  ...
  fit_data <- Maaslin2(
    input_data = t(otu_table(phy_list_2[[i]])), 
    input_metadata = df_input_metadata, 
    output = out_dir, 
    fixed_effects = c("DiseaseCausative", "Timeperiods"),
    random_effects = c("Patient.ID")
...
#Pairwise causative pathogen comparrison:

Compare each type to the control group:
```{r}
#subset by each causative pathogen paired with control groups:
phy.red_SA_N= subset_samples(phy.red, Causative_pathogen== c("Staphylococcus aureus", -99))
phy.red_EF_N= subset_samples(phy.red, Causative_pathogen== c("Enterococcus faecalis",-99))
phy.red_EC_N= subset_samples(phy.red, Causative_pathogen== c("Escherichia coli",-99))
phy.red_KO_N= subset_samples(phy.red, Causative_pathogen== c("Klebsiella oxytoca",-99))
phy.red_KP_N= subset_samples(phy.red, Causative_pathogen== c("Klebsiella pneumoniae",-99))
phy.red_ECL_N= subset_samples(phy.red, Causative_pathogen== c("Enterobacter cloacae",-99))
phy.red_EA_N= subset_samples(phy.red, Causative_pathogen== c("Enterobacter aerogenes",-99))

#create a list of these:
phy_list_cause=list(phy.red_SA_N, phy.red_EF_N, phy.red_EC_N,phy.red_KO_N, phy.red_KP_N,phy.red_ECL_N, phy.red_EA_N)
names(phy_list_cause)=c("SA_N","EF_B","EC_N", "KO_N", "KP_N", "ECL_N", "EA_N")

#compare pairwise by causative pathogen:
for (i in 1:length(phy_list_cause)){
fit_data = Maaslin2(
  input_data = t(otu_table(phy_list_cause[[i]])), 
  input_metadata = df_input_metadata, 
  output = paste0("Maaslin2_causative_pathogen_comp_output_",names(phy_list_cause)[i]), 
  fixed_effects = c("Causative_pathogen", "Timeperiods"),
  random_effects = c("Patient.ID"))
}
```

Compare the ones who were different in alpha diversity:
```{r}
#subset by pairs:
phy.red_SA_EC= subset_samples(phy.red, Causative_pathogen== c("Staphylococcus aureus","Escherichia coli"))
phy.red_EA_KP= subset_samples(phy.red, Causative_pathogen== c("Enterobacter aerogenes","Klebsiella pneumoniae"))
phy.red_ECL_KP= subset_samples(phy.red, Causative_pathogen== c("Enterobacter cloacae","Klebsiella pneumoniae"))
phy.red_SA_EF= subset_samples(phy.red, Causative_pathogen== c("Staphylococcus aureus", "Enterococcus faecalis"))
phy.red_EC_KP= subset_samples(phy.red, Causative_pathogen== c("Escherichia coli","Klebsiella pneumoniae"))
phy.red_SA_KP= subset_samples(phy.red, Causative_pathogen== c("Staphylococcus aureus","Klebsiella pneumoniae"))

#create a list of these subsets:
phy_list_cause_2=list(phy.red_SA_EC, phy.red_EA_KP, phy.red_ECL_KP,phy.red_SA_EF, phy.red_EC_KP,phy.red_SA_KP)
names(phy_list_cause_2)=c("SA_EC","EA_KP","ECL_KP", "SA_EF", "EC_KP", "SA_KP")

for (i in 1:length(phy_list_cause_2)){
fit_data = Maaslin2(
  input_data = t(otu_table(phy_list_cause_2[[i]])), 
  input_metadata = df_input_metadata, 
  output = paste0("Maaslin2_alpha_diversity_output_",names(phy_list_cause_2)[i]), 
  fixed_effects = c("Causative_pathogen", "Timeperiods"),
  random_effects = c("Patient.ID"))
}
```

#Gram negative comparison without e.coli:
```{r}
#remove all gram neg caused by e.coli:

phy.red.red = subset_samples(phy.red, Bloodculture_isolated_pathogen!= "Escherichia coli")

#extract their info only:
df_input_metadata_red =data.frame(sample_data(phy.red.red))
df_input_metadata_red$Timeperiods = factor(df_input_metadata_red$Timeperiods,
                                   levels = c("A", "B", "C", "D"))

#reformat the inputs as strings:
df_input_metadata_red$DiseaseCausative[df_input_metadata_red$DiseaseCausative == 1 ] <- "s.aureus"
df_input_metadata_red$DiseaseCausative[df_input_metadata_red$DiseaseCausative == 2 ] <- "gram.neg"
df_input_metadata_red$DiseaseCausative[df_input_metadata_red$DiseaseCausative == 0 ] <- "none"

df_input_metadata_red$DiseaseCausative = factor(df_input_metadata_red$DiseaseCausative,
                                   levels = c("s.aureus", "gram.neg", "none"))

#subset by pairs of LOS types:
phy.red_SG_red= subset_samples(phy.red.red, DiseaseCausative!= 0)
phy.red_SN_red= subset_samples(phy.red.red, DiseaseCausative!= 2)
phy.red_NG_red= subset_samples(phy.red.red, DiseaseCausative!= 1)

#create a list of the subsets:
phy_list_3=list(phy.red_SG_red, phy.red_SN_red, phy.red_NG_red)
names(phy_list_3)=c("SG_red","SN_red","NG_red")

#compare pairwise across LOS type:
for (i in 1:length(phy_list_3)){
fit_data = Maaslin2(
  input_data = t(otu_table(phy_list_3[[i]])), 
  input_metadata = df_input_metadata_red, 
  output = paste0("Maaslin2_Gram_negative_comp_output_",names(phy_list_3)[i]), 
  fixed_effects = c("DiseaseCausative", "Timeperiods"),
  random_effects = c("Patient.ID"))
}
